...
import { interpretReport } from './ExecutiveReportInterpreter';

export interface FounderTurnContext {
  founderName?: string;
  companyName?: string;
...
export class ContextBuilder {
   ...
   */
  public static buildContextPackage(context: FounderTurnContext): string {
    ...
    if (assessment.questionnaire && Array.isArray(assessment.questionnaire)) {
      questionnaireInsights = assessment.questionnaire.map((q: any) => `Question: "${q.question}" -> Answer: "${q.answer}"`).join("\n");
    }
    ...
    if (context.history && context.history.length > 0) {
      ...
      if (histText.includes("decision") && !discussed.includes("Decision Load")) discussed.push("Decision Load");
      if (histText.includes("execution") && !discussed.includes("Execution")) discussed.push("Execution");
      if (histText.includes("leadership") && !discussed.includes("Leadership Alignment")) discussed.push("Leadership Alignment");
    }
