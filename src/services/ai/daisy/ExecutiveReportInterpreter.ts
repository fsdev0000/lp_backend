 ...
 */
export const interpretReport = (vars: Record<string, any>): ExecutiveInterpretation => {
  const primaryFinding = vars.primaryPressureArea || (vars.pressureDimensions && vars.pressureDimensions[0]) || "Founder Dependency";

  let topBottlenecks: string[] = [];
  if (Array.isArray(vars.pressureDimensions) && vars.pressureDimensions.length > 0) {
    topBottlenecks = vars.pressureDimensions.slice(0, 3).map((d: string) => `Operational bottleneck centering around ${d}`);
  } else {
  ...
  const topRecommendations: string[] = Array.isArray(vars.recommendations) && vars.recommendations.length > 0
  ...
  const bookingReason = 
    `To examine the structural causes behind ${primaryFinding} and establish operational alignment during a Strategic Review with Lionel Eersteling.`;
  ...
  const storedExplanations: Record<string, string> = {
...
export const getExecutiveReportPrompt = (interpretation: ExecutiveInterpretation): string => `## EXECUTIVE REPORT INTERPRETATION & DYNAMIC AI PROHIBITIONS
