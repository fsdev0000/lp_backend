import { getFreeSlots, bookAppointment } from '../ghl';
...
export async function executeBackendTool(name: string, args: any, contactId: string): Promise<string> {
  try {
    switch (name) {
      case 'get_available_times':
        const slots = await getFreeSlots(args.date);
        return JSON.stringify({ slots });
        
      case 'book_appointment':
        if (!contactId) return JSON.stringify({ error: "Missing contactId for booking." });
        const result = await bookAppointment(contactId, args.dateTime, args.title);
