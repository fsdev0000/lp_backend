...
async function getGHLConfig(): Promise<GHLConfig> {
  const ghlBase =
    process.env.GHL_BASE ||
    (await getSecret('GHL_BASE')) ||
    'https://services.leadconnectorhq.com';
  const ghlApiKey =
    process.env.GHL_API_KEY ||
    (await getSecret('GHL_API_KEY'));
  const ghlLocationId =
...
function normalizePayload(body: any): ContactPayload {
  ...
  if (preferredMethod) {
    ...
    if (lower === 'email') preferredMethod = 'Email';
    else if (lower === 'phone') preferredMethod = 'Phone';
    else if (lower === 'whatsapp') preferredMethod = 'WhatsApp';
    else if (lower === 'phone or whatsapp' || lower === 'phone/whatsapp') preferredMethod = 'Phone or WhatsApp';
  }
...
function validatePayload(payload: ContactPayload): string | null {
  ...
  if (!nameCharRegex.test(firstName)) return 'First name can only contain letters, spaces, hyphens, and apostrophes.';
  ...
  if (!nameCharRegex.test(lastName)) return 'Last name can only contain letters, spaces, hyphens, and apostrophes.';

  const company = payload.company || '';
  if (!company) return 'Please provide your company or organisation.';
  ...
  if (!companyCharRegex.test(company)) return 'Company name contains invalid characters.';

  const role = payload.role || '';
  if (!role) return 'Please provide your role or title.';
  if (role.length < 2) return 'Role must be at least 2 characters.';
  if (role.length > 100) return 'Role cannot exceed 100 characters.';
  if (!roleCharRegex.test(role)) return 'Role contains invalid characters.';

  const email = payload.email || '';
  if (!email) return 'Please provide your email address.';
  if (email.length > 254 || !emailRegex.test(email)) return 'Please provide a valid email address (e.g. name@company.com).';
  ...
  if (phone) {
    ...
    if (!phoneCharRegex.test(phone)) return 'Please provide a valid phone number (e.g. +971 50 123 4567).';
  }

  const message = payload.message || '';
  if (!message) return 'Please describe what you are preparing for.';
  if (message.length < 10) return 'Description must be at least 10 characters.';
...
async function upsertGHLContact(payload: ContactPayload, config: GHLConfig): Promise<string | null> {
...
async function sendInternalNotificationEmail(payload: ContactPayload, config: GHLConfig): Promise<void> {
  const locationId = config.ghlLocationId;
  ...
  for (const recipient of NOTIFICATION_RECIPIENTS) {
    try {
      ...
      const adminData: any = await adminUpsertRes.json();
 ...
 */
async function handleContact(req: Request, res: Response): Promise<void> {
  try {
