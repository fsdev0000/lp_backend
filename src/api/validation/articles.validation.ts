...
export function parseAndValidateArticleInput(
  ...
  if (typeof rawBody === 'object' && rawBody !== null && !Buffer.isBuffer(rawBody)) {
    ...
    if (trimmed.startsWith('{') && trimmed.endsWith('}')) {
      ...
      body = {
        ...
        keywords: getHeader('x-keywords') ? getHeader('x-keywords')!.split(',').map((k: string) => k.trim()) : undefined,
  ...
  if (typeof content === 'string') {
    ...
    if (fmMatch) {
      ...
      if (reading_time === undefined && fm.reading_time) reading_time = Number(fm.reading_time);
      if (keywords === undefined && fm.keywords) keywords = fm.keywords.split(',').map((k: string) => k.trim()).filter(Boolean);
      if (published === undefined && fm.published !== undefined) published = fm.published === 'true';
      if (publish_date === undefined && fm.publish_date) publish_date = fm.publish_date;
      if (pillar_color === undefined && fm.pillar_color) pillar_color = fm.pillar_color;
